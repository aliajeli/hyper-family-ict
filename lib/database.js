...
// Read JSON file (No Cache Logic)
async function readJsonFile(filename) {
  await ensureDbExists();
  const filePath = path.join(DB_PATH, `${filename}.json`);

  try {
    // خواندن فایل به صورت خام در هر بار درخواست
...
export const db = {
  ...
  destinations: {
    ...
    async delete(id) {
      console.log(`🧹 Database: Deleting ID ${id}`); // 1. آیا این چاپ می‌شود؟

      const destinations = await this.getAll();
      const filtered = destinations.filter((d) => d.id !== id);

      console.log(`📊 Filtered Count: ${filtered.length}`); // 2. این چطور؟

      // 👇 این خط حیاتی است
      await writeJsonFile("destinations", filtered);

      console.log("💾 Write called"); // 3. و این؟
