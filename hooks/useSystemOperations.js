...
export const useSystemOperations = () => {
  ...
  const manageService = async (ip, serviceName, action) => {
    console.log(
      `🔧 Calling Service Manager: ${action} ${serviceName} on ${ip}`,
    ); // 👈 این لاگ را اضافه کنید
  ...
  const copyFileSecure = async (src, destIp, destPath) => {
    ...
    if (localHash.success && remoteHash.success) {
      ...
      addLog(
        `Verification Failed: ${remoteHash.error || localHash.error}`,
        "error",
      );
      return "retry";
    }
  ...
  const addLog = (message, type = "default") => {
    ...
    // 2. File Log (Background)
    if (window.electron) {
      // حذف کدهای رنگی یا فرمتینگ اضافه اگر لازم است
      window.electron.logToFile(`[${type.toUpperCase()}] ${message}`);
    }
  };

  const clearLogs = () => setLogs([]); // 👈 تابع جدید برای پاک کردن
  return {
    logs,
    clearLogs, // 👈 Export
