...
const DeleteModal = ({ isOpen, onClose }) => {
  ...
  const handleStart = async () => {
    if (selectedFiles.length > 0 && selectedTargets.length > 0) {
      ...
      const completed = await startOperation("delete", {
        files: selectedFiles,
              ...
              <div className="flex-1 overflow-y-auto p-1.5 space-y-1 custom-scrollbar h-full">
                ...
                {selectedFiles.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-50 gap-1">
                    <Trash2 className="w-8 h-8" strokeWidth={1} />
            ...
            <div className="h-16 bg-[#1e293b] border border-slate-700 rounded-xl px-4 flex justify-between items-center shadow-lg shrink-0">
              <div className="flex flex-col">
                ...
                <div className="text-[9px] text-slate-500 font-mono mt-0.5">
                  {isRunning ? "STATUS: DELETING..." : "STATUS: IDLE"}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {isRunning ? (
                  <Button
                    ...
                    className="h-10 px-6 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white border border-red-500/20"
                  >
                    Stop
                  </Button>
                ) : (
                  <Button
                    onClick={handleStart}
                    className={cn(
                      "h-10 px-8 text-white text-xs font-semibold tracking-wide rounded-lg border border-rose-500/30",
                      "bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500",
                      "shadow-[0_0_10px_rgba(244,63,94,0.3)] hover:shadow-[0_0_15px_rgba(244,63,94,0.5)]",
                      (selectedFiles.length === 0 ||
                        selectedTargets.length === 0) &&
