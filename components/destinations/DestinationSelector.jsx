...
const DestinationSelector = ({ className }) => {
  ...
  if (destinations.length === 0) {
    return (
      <div
        className={cn(
          "bg-bg-tertiary rounded border border-border p-4 text-center text-xs text-text-muted",
          className,
        )}
      >
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full bg-bg-tertiary rounded border border-border",
        className,
      )}
    >
      {/* Header / Toolbar */}
      <div className="flex items-center justify-between p-2 border-b border-border bg-bg-secondary">
        <span className="text-xs font-medium text-text-secondary">
          Target Systems ({selectedDestinations.length})
        </span>
